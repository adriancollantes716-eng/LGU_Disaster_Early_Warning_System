...
## Role purpose
The Database Architect & RBAC Lead owns the data model and the row-level access rules that prevent users from reading or changing records outside their authority. In this system, this role makes the database useful for disaster operations while defending against IDOR-style access bypasses.
