...
## Role purpose
The API & IAM Engineer owns the Django REST Framework API and the identity/access rules that decide who can call each endpoint. In this system, this role connects authentication, JWT tokens, role checks, serializers, and API documentation into one secure backend contract.
...
- `carigara/settings.py` controls DRF, JWT, authentication classes, pagination, filters, and schema generation.
- `carigara/urls.py` exposes token, schema, Swagger, account, and incident routes.
...
- `API_DOCUMENTATION.md` and `Carigara_API_Collection.postman_collection.json` document and exercise the API.
...
- `carigara/settings.py` registers `AUTH_USER_MODEL = 'accounts.User'` so the whole project uses that custom user.
...
- `carigara/urls.py` exposes `/api/token/` for login token creation and `/api/token/refresh/` for token renewal.
