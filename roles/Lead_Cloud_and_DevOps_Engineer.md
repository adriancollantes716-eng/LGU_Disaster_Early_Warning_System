...
## Role purpose
The Lead Cloud & DevOps Engineer owns the production path for the system: runtime, deployment, environment variables, database connection, static files, uploaded media, and cloud hosting behavior.
...
- `carigara/settings.py` reads environment variables and configures database, hosts, storage, security, CORS, and logging.
...
- `carigara/settings.py` builds `ALLOWED_HOSTS` from the environment and adds Render host support.
...
- Logging creates a local `logs` directory and writes warnings to `logs/carigara.log`.
...
- Confirm `gunicorn carigara.wsgi:application` can start the app in production mode.
