...
class HazardAdmin(admin.ModelAdmin):
    ...
    filter_horizontal = ()
    readonly_fields = ('created_at', 'updated_at')
    actions = ['set_alert_red', 'set_alert_orange', 'set_alert_amber', 'set_alert_yellow', 'set_alert_green']
    ...
    def set_alert_amber(self, request, queryset):
        """Bulk action: Set selected hazards to AMBER alert level"""
        updated = queryset.update(alert_level='amber')
        self.message_user(request, f'Successfully updated {updated} hazard(s) to AMBER alert level.')
        
        IncidentBulkUpdate.objects.create(
            admin_user=request.user,
            update_type='hazard_status',
            filter_criteria={'ids': list(queryset.values_list('id', flat=True))},
            update_data={'alert_level': 'amber'},
            status='completed',
            records_affected=updated
        )
    set_alert_amber.short_description = 'Set selected hazards to AMBER (Elevated Risk)'
...
class IncidentAdmin(admin.ModelAdmin):
    ...
    def mark_confirmed(self, request, queryset):
        ...
        # Capture current instances so we can notify about their previous status
        incidents = list(queryset.select_related('reported_by', 'assigned_to'))
        old_status_map = {inc.id: inc.status for inc in incidents}

        updated = queryset.update(status='confirmed')

        # Notify each affected incident about the status change
        try:
            from .views import _send_incident_status_change_email
            for inc in incidents:
                old_status = old_status_map.get(inc.id)
                inc.status = 'confirmed'
                try:
                    _send_incident_status_change_email(inc, old_status, request)
                except Exception:
                    # Keep admin action resilient; log optionally in future
                    pass
        except Exception:
            pass
