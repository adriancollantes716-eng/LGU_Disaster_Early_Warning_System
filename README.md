# Municipality of Carigara, Leyte Disaster Early Warning System

Local Government Disaster Early Warning and Incident Management System for Carigara, Leyte.

This is a Django-based web and API application for monitoring hazards, logging incidents, managing response workflows, and keeping an audit trail of system usage. It is designed for role-based municipal operations: superadmin, admin, dispatcher, and public viewer.
...
project-root/
  ...
  carigara/              Project settings, URLs, WSGI/ASGI
...
For end users, see the full user guide in `USER_GUIDE.md`.
...
```bash
python manage.py migrate --noinput && python setup_demo_data.py && gunicorn carigara.wsgi:application --bind 0.0.0.0:$PORT
```
...
- `Carigara_API_Collection.postman_collection.json` - Postman collection
